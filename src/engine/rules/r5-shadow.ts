...
function isHeuristicShadow(t: EndpointTemplate, all: EndpointTemplate[]): boolean {
  if (SHADOW_PREFIX.test(t.template)) return true;
  const counts = [...all.map((x) => x.requestCount)].sort((a, b) => a - b);
...
export function detectR5Shadow(ctx: DetectionContext): Finding[] {
  ...
  const shadowTemplates = specProvided
    ? ctx.templates.filter((t) => !t.documented)
    : ctx.templates.filter((t) => isHeuristicShadow(t, ctx.templates));
    ...
    return {
      ...
      rationale: specProvided
        ...
        : `${t.template} received ${t.requestCount} request(s) and matches the shadow-endpoint heuristic ` +
          `(internal/debug/legacy path prefix or low-traffic with no OPTIONS/HEAD support) — no spec was imported ` +
